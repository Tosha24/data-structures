...
public:
    static int maximumInArray(vector<int>& nums){
        int max = 0;
        for(int i : nums){
            if(i > max){
                max = i;
    ...
    static int sumOfArray(vector<int>& nums){
        int sum = 0;
        for(int i:nums){
            sum = sum + i;
        }
    ...
    static int findPieces(vector<int>& nums, int mid){
        int sum = 0;
        int pieces = 0;
        int i = 0;
        while(i < nums.size()){
    ...
    static int splitArray(vector<int> &nums, int k){
...
int main()
{
    ...
    int k = 5;
    cout << _410_SplitArrayLargestSum::splitArray(nums, k) << endl;
    return 0;
}
